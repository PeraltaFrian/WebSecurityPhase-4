...
import DOMPurify from 'dompurify'; //Imported for sanitizing
import './FileView.css'
                        ...
                        {files.map((file) => {
                            return (
                                <tr className="list-item" key={file._id}>
                                    {/* changed <td className="txt">{file.filename}</td> */}
                                    <td className="txt" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(file.filename) }}></td>
